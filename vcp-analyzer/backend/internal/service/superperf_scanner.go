...
type SuperPerfScanParams struct {
	...
	GainPeriod   string  // "1m", "3m", "6m", "ytd" — sort by this gain, default "3m"
...
func (s *SuperPerfScanner) AnalyzeAll(
	...
	type intermediate struct {
		...
		gain1m, gain3m, gain6m, gainYTD float64
		price  float64
		adv20  float64
	}
	...
	for _, chart := range charts {
		...
		pool = append(pool, intermediate{
			chart: chart, gain1m: gain1m, gain3m: gain3m,
	...
	for rank, item := range pool {
		analysis := s.scoreStock(item.chart, item.price, item.adv20,
			item.gain1m, item.gain3m, item.gain6m, item.gainYTD,
			rank, len(pool))
		if analysis != nil {
